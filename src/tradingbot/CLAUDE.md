...
`tradingbot` 파이썬 패키지의 루트. Upbit KRW 스팟 전용 백테스트·ML·라이브 엔진 + Typer 기반 CLI(`cli.py` 24 commands). 서브패키지가 책임을 나눠 갖고, `cli.py` 가 모든 진입점을 모은다.
...
- **CLI lazy import 컨벤션**: `cli.py` 가 ~4200 줄이라 모듈 import 가 전부 top-level 이면 모든 명령이 무거워진다. 무거운 import(ML/ccxt/streamlit)는 함수 안에서.
