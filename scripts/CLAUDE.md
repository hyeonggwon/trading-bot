...
- **새 PC 부트스트랩**: `setup.sh` — venv 생성(Python 3.12 필수 — `constraints.txt` freeze 기준, `PYTHON=` 환경변수로 지정)·`constraints.txt` 고정 설치·`.env` 복사(기존 파일 보존)·`core.hooksPath` 설정을 idempotent 하게 수행. 데이터/모델 재생성은 명령 안내만 출력.
