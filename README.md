...
- **ML 전략 (LightGBM)** — 10 기술 피처 + 6 외부 피처(김프/funding/FNG/USD_KRW) + 12 추가 피처(레짐/lag/세션, opt-in), 다중 타깃 옵션(binary / ATR / triple-barrier), Walk-Forward 학습 + 아이소토닉 확률 보정, Half-Kelly 포지션 사이징(학습 메타의 실측 win/loss 비율 사용), 시간 정직한 `ml-walk-forward` (윈도우마다 모델 재학습), Optuna 하이퍼파라미터 튜닝(`ml-tune` / `ml-tune-all`), 모델별 entry/exit 임계값 튜닝(`ml-tune-thresholds*`)
...
# 진단 리포트 (캘리브레이션 / 피처 중요도 / WF Sharpe 한 번에)
tradingbot ml-diagnostics --symbol BTC/KRW --timeframe 4h

# 추가 피처 / 다른 타깃으로 학습
tradingbot ml-train --symbol BTC/KRW --timeframe 4h \
  --target-kind triple-barrier --atr-mult 1.0 --include-extra

# Optuna 하이퍼파라미터 튜닝 (단일 / 전체 모델)
tradingbot ml-tune --symbol BTC/KRW --timeframe 4h --trials 50 --time-budget 3600
tradingbot ml-tune-all --workers 4

# 모델별 entry/exit 임계값 그리드 튜닝 (재학습 없이 meta 갱신)
tradingbot ml-tune-thresholds --symbol BTC/KRW --timeframe 4h
...
| `lgbm` | LightGBM ML 메타 모델 (10 기술 + 6 외부 피처, +12 옵션 피처 / 다중 타깃 / Optuna 튜닝 / per-model 임계값 → Half-Kelly with 실측 win/loss) | `entry_threshold` (기본 0.45, meta 우선), `exit_threshold` (기본 0.30, meta 우선), `external_data_dir` (자동 주입) |
...
│   │
│   ├── ml/                           # 머신러닝
│   │   ├── features.py               #   피처 엔지니어링 (10 기술 + 6 외부 + 12 옵션 추가)
│   │   ├── targets.py                #   타깃 옵션 (binary / ATR / triple-barrier)
...
│   │   ├── diagnostics.py            #   캘리브레이션/Brier/ECE/예측분포 진단 헬퍼
│   │   ├── tuner.py                  #   Optuna 하이퍼파라미터 튜너
│   │   ├── threshold_tuner.py        #   per-model entry/exit 임계값 그리드 튜너
│   │   ├── parallel.py               #   병렬 워커 (학습/튜닝/임계값)
